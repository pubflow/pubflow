// examples/react/banking-example.tsx
import React, { useState, useEffect } from 'react';
import { PubFlow, PubFlowProvider, useBridge } from 'pubflow/react';
      ...
      const fetchRelated = async () => {
        try {
          ...
          if (transaction.relatedTransactionId) {
            const response = await fetch(`/api/transactions/search?q=id:${transaction.relatedTransactionId}`);
            const relatedTxn = await response.json();
            setRelatedTransaction(relatedTxn);
          }
          
          if (transaction.relatedAccountId) {
            const response = await fetch(`/api/accounts/search?q=id:${transaction.relatedAccountId}`);
